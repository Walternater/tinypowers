...
- SPEC-STATE 阶段推进禁止跳步
- 技术方案完成后必须通过显式确认，不能用普通文字代替
- 已确认决策在 `/tech:code` 阶段不能被擅自推翻
- 任务拆解完成后也必须显式确认，不能直接流入 `/tech:code`
...
- 跳过歧义检测直接做技术方案 → 方案在实现时发现需求冲突：歧义检测的高优先级项必须清零才能进入 DESIGN
- 技术方案不做用户确认就拆任务 → 返工：方案完成后必须显式确认
- 任务粒度过大 → 执行时无法评估进度：Task 必须 ≤ 1 人天
- 不探索上下文直接设计 → 可能与现有模式冲突：Phase 1 必须先了解项目上下文
- 单方案直接实现 → 用户失去选择权：必须提出 2-3 方案 + trade-offs
