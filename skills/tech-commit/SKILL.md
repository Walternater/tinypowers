...
本 skill 是 **tinypowers 独有**（无 superpowers 方法论引用）。

## 复杂度路由

| 模式 | 判定条件 | 流程差异 |
|------|---------|---------|
| **Fast** | 单模块改动、无远程仓库或无需 PR | Step 1+2 合并、跳过 Step 3（PR） |
| **Standard** | 多模块、有远程仓库 | 完整 3 步流程 |
...
```text
Step 1: Document Sync + Knowledge Capture（Fast: 合并 / Standard: 分两步）
Step 2: Git Commit
Step 3: PR + Branch Cleanup（Standard only）
```
...
## 1. Document Sync + Knowledge Capture
...
> 原 Constraint/Rejected/Evidence/Confidence trailer 字段已移除。这些信息应在技术方案.md 中记录，不应在 commit message 中重复。
...
## 3. PR + Branch Cleanup（Standard only）

> Fast 模式：无远程仓库或无需 PR 时跳过此步。
...
如果仓库采用 Keep a Changelog 风格，优先按 `Added / Changed / Fixed / Security` 归类。

### Branch Cleanup

- 决定分支去留（merge / keep / discard）
- 清理 worktree（如果使用了隔离环境）
- 确认工作区恢复干净状态
...
docs/knowledge.md          ← 项目级知识沉淀（Step 1 更新）
