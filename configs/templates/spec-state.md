...
| executing | EXEC | SPEC-STATE.md 已创建、Wave 执行中 |
...
> - `ready`：规划完成，可进入 EXEC
>
> `current_wave` / `exec_progress` / `blockers` 在 `phase: EXEC` 时使用：
> - `current_wave`：当前执行 Wave 编号
> - `exec_progress`：已完成/总任务数
> - `blockers`：阻塞项列表
...
EXEC -> REVIEW:   SPEC-STATE.md 已收敛，审查与验证证据齐备
