...
这份文档定义 tinypowers 在当前正式支持宿主里的运行方式，避免安装、命令命名、hook 事件和可用能力各写一套口头约定。
...
|------|-------------|-------|
| 初始化 | `/tech:init` | 通过 skills / prompt 调用同名能力 |
| 需求分析 | `/tech:feature` | 同上 |
| 执行与审查 | `/tech:code` | 同上 |
| 收口提交 | `/tech:commit` | 同上 |
...
| Context | Claude Code | Codex |
|---------|-------------|-------|
| `dev` | 通过 skills 和 rules 引导 | 通过 SKILL.md discovery 引导 |
| `research` | 同上 | 同上 |
| `review` | 同上 | 同上 |
...
| 能力 | Claude Code | Codex |
|------|-------------|-------|
| Session 生命周期 | 直接映射 `SessionStart / Stop / PreCompact` | 视宿主能力决定是否启用 |
| PostToolUse 监控 | 原生支持 | 取决于宿主是否支持相似 hook 机制 |
| Strict 验证提醒 | 支持 | 可降级为显式脚本调用 |
