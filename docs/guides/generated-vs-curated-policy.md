...
- Curated Content 不等于“默认安装一定复制的全部内容”
- 默认安装面只覆盖 runtime essentials
- `docs/plans/`、workflow optimization 文档、`tests/` 这类仓库维护材料可以仍属于 Curated，但应作为显式 opt-in 组件安装
...
- 如果它属于仓库维护材料而不是目标项目运行时必需内容，应优先拆成可选组件，而不是默认安装。
...
- 默认 profile 只带 runtime essentials
- `repo-maintenance`、归档文档、测试目录保持显式 opt-in
