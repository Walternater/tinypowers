...
主要步骤：
- 先运行 `npm run commit:prepare-docs` 与 `npm run commit:check-docs`，脚本会自动识别当前 feature；只有存在歧义时才需要补 `--feature`
- 再根据代码改动同步必要文档，包括 README / 部署说明、ReDoc / OpenAPI / 接口说明，以及必要的 API / 数据库文档
- 推进 `SPEC-STATE -> DONE`
