...
function handleSessionStart(data, snapshotPath, cwd) {
  ...
  // Inject notepad content for compaction resistance (if exists)
  const notepadPath = path.join(cwd, '.tinypowers', 'notepad.md');
  let notepadNote = '';
  if (fs.existsSync(notepadPath)) {
    const notepadContent = fs.readFileSync(notepadPath, 'utf8');
    notepadNote = '\n\n---\n.tinypowers/notepad.md 已存在，上次会话的关键状态已持久化。\n' +
      '读取 .tinypowers/notepad.md 可获取压缩前的恢复信息。';
  }

  const output = {
    hookSpecificOutput: {
      hookEventName: "SessionStart",
      additionalContext: message + notepadNote
    }
  };
...
function handleStop(data, snapshotPath, cwd) {
  ...
  // Write persistent handoff to .tinypowers/ (survives session death)
  writePersistentHandoff(cwd, featureId, summary, data.session_id);

  // Write notepad for compaction resistance
  writeNotepad(cwd, featureId, summary);
}

function writePersistentHandoff(cwd, featureId, summary, sessionId) {
  const tpDir = path.join(cwd, '.tinypowers');
  ...
  const handoff = {
    ...
    decisions: summary.decisions || [],
    last_action: summary.last_action || '',
    progress_pct: summary.total_tasks > 0
      ? Math.round((summary.completed_tasks / summary.total_tasks) * 100)
      : 0
  };

  fs.writeFileSync(
    path.join(tpDir, 'handoff.json'),
...
function writeNotepad(cwd, featureId, summary) {
  ...
  const notepadContent = `<!-- notepad.md | auto-generated by tinypowers-session-manager -->
<!-- DO NOT edit manually — this file survives /compact and session resets -->

## Session Recovery Notepad

**Feature:** ${featureId}
**Last Updated:** ${new Date().toLocaleString()}
**Session Phase:** ${summary.phase || 'unknown'}
**Wave:** ${summary.wave || '?'} / ${summary.total_waves || '?'}
**Tasks:** ${summary.completed_tasks || 0} / ${summary.total_tasks || 0} completed

## Critical State (DO NOT LOSE)

${stateMdContent ? '```\n' + stateMdContent.slice(0, 500) + '\n```\n' : '_No STATE.md content captured_'}
...
${summary.last_action ? `> ${summary.last_action}` : '_Read STATE.md to determine next step_'}

---
*This notepad is injected into context on session resume. Keep it concise.*
`;
...
function handlePreCompact(data, snapshotPath, cwd) {
  ...
  // Write persistent notepad before compaction (key for compaction resistance)
  writeNotepad(cwd, featureId, summary);
...
.notepad.md 和 .tinypowers/handoff.json 已同步更新。
压缩后将自动恢复现场。`
    }
  };
